...
// fix console errors from here down
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.messages && response.messages.length > 0) {
                if (response.deleted) {
                    alert("Some cards have been deleted from the database by another adminstrator: " + response.messages.join(", ") +". These card were not updated, please manually restore them using the Add function");
                } else {
...
$("#change-lesson").click(function() {
    ...
    $.ajaxSetup({
     ...
     $.ajax({
            ...
            contentType: 'application/json',
            success: function alertsuccess(request)  {
